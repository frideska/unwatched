...
let getTvForUser = async(tvID, user, type) => {
  ...
  if(tvKey){
    if(type==='library') {
      ...
      let userTv = await WatchlistUserTv.findOne({tv_id: tvKey, user_id: user._id})
      return userTv ? userTv: false
    }
  }
  return false
...
let getTvId = async(tvKey) => {
  const result =  await Tv.findOne({_id: tvKey})
  const id = result.tv_id
  return id
}
...
let newTv = async (tvID, user) => {
  ...
  if (tvWatchlist && tvKey) {
    try {
      await WatchlistUserTv.remove({tv_id: tvKey, user_id: user._id})
    } catch (err) {
      console.error(err)
      return false
    }
...
let findTvForUser = async (user) => {
  ...
  try {
    return clean(await Promise.all(userTv.map(async (tv) => {
      const tvID = await getTvId(tv.tv_id)
      const watchlist = false
      const library = true
