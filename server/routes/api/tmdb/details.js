...
route.get('/movie/:id', async (req, res) => {
  const watchlist = await WatchlistMovieController.movieInWatchlist(req.params.id, req.user)
  const library = await LibraryMovieController.movieInLibrary(req.params.id, req.user)
  res.send(await tmdbWrapper.details.movie(req.params.id, watchlist, library))
})
route.get('/tv/:id', async (req, res) => {
  const watchlist = await WatchlistSeriesController.tvInWatchlist(req.params.id, req.user)
  const library = await LibrarySeriesController.seriesInLibrary(req.params.id, req.user)
